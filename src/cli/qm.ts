...
const AYUDA = `qm · cuánta cuota te queda, en todos tus perfiles de Claude Code
  ...
  qm --sin-antigravity  no mira la cuenta de Antigravity
...
function parsearArgs(argv: readonly string[]): Opciones | string {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    else if (a === '--sin-antigravity') o.antigravity = false;
...
interface FilaPerfil {
  ...
  producto: 'claude' | 'codex' | 'opencode' | 'antigravity';
  perfil: Perfil;
  veredicto: string;
...
// La cuenta de Antigravity como una fila más.
//
// El piso local sale de las bases por conversación en
// ~/.gemini/antigravity/conversations/<uuid>.db, donde cada request deja su
// desglose de tokens. Ver `adapters/antigravity.ts` para de dónde salen esos
// números y por qué hay un gate de forma antes de creerles.
//
// Antigravity no reporta cuota porcentual ni reinicios —se buscó: el estado no
// la trae y en los logs del IDE la única mención es `quota undefined`—, así que
// se reporta `sin-cuota-legible`, igual que los planes de API key de opencode.
function filaAntigravity(o: Opciones): FilaPerfil | null {
  if (!hayAntigravity()) return null;

  const perfil = perfilAntigravity();
  const cred = estadoCredencialAntigravity();
  ...
  const local = o.breve ? null : consumoAntigravity(corteDias);
  const ventana = o.breve ? null : consumoAntigravity(corteVentana);

  // Un formato que cambió NO es consumo cero. Si el gate de forma rechazó todo
  // lo que había, la fila lo dice en vez de mostrar un 0 tranquilizador.
  const cuota: ResultadoCuota = local?.formaRota
    ? {
        estado: 'sin-cuota-legible',
        detalle: 'el formato de Antigravity cambió: los tokens no se pueden leer',
      }
    : {
        estado: 'sin-cuota-legible',
  ...
  return {
    producto: 'antigravity',
    localMedido: local !== null && !local.formaRota,
    ...
    ultimoUso: ultimaActividadAntigravity(),
  };
}
...
async function medir(o: Opciones): Promise<FilaPerfil[]> {
  ...
  const antigravity = o.antigravity ? filaAntigravity(o) : null;
  const todas = [
    ...claude,
    ...(codex === null ? [] : [codex]),
    ...(antigravity === null ? [] : [antigravity]),
