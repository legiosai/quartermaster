<!--
  BITACORA.md — el README que este repo tuvo hasta 0.1.6, entero.

  No es documentación de uso: eso está en el README (inglés) y en LEEME.md
  (castellano). Esto es el registro de cómo se llegó a cada decisión, con los
  errores adentro — los tres intentos de arquitectura del panel de GNOME, la
  suposición sobre la barra de menú que resultó falsa, el gate que miraba una
  función de 1500 líneas, el paquete de npm que instalaba y no arrancaba.

  Se separó porque son dos cosas distintas con dos lectores distintos: alguien
  que llega de un catálogo quiere saber qué instala en dos minutos, y la
  bitácora no le contesta eso. Pero es el mejor material del repo y no se toca:
  cada sección tiene un número medido atrás, y varias explican por qué NO se
  hizo lo obvio.

  This is the engineering log, in Spanish. It is kept as-is on purpose: it
  records how each decision was reached, mistakes included. For what the tool
  does and how to install it, see README.md.
-->
